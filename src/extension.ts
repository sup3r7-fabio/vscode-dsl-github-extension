...
import { DslDebugAdapterDescriptorFactory } from './debugAdapter';

let dapServerProcess: ChildProcess | null = null;
...
export function activate(context: vscode.ExtensionContext) {
    ...
    const factory = new DslDebugAdapterDescriptorFactory();
    context.subscriptions.push(
        vscode.debug.registerDebugAdapterDescriptorFactory('dsl', factory)
    );
...
function registerCommands(context: vscode.ExtensionContext) {
    ...
    const startDapServerCommand = vscode.commands.registerCommand(
        'github-actions-dsl.startDapServer',
        startDapServer
    );

    // Stop DAP Server command
    const stopDapServerCommand = vscode.commands.registerCommand(
        'github-actions-dsl.stopDapServer',
        stopDapServer
    );

    // Restart DAP Server command
    const restartDapServerCommand = vscode.commands.registerCommand(
        'github-actions-dsl.restartDapServer',
        async () => {
            await stopDapServer();
    ...
    const openDebugConsoleCommand = vscode.commands.registerCommand(
        'github-actions-dsl.openDebugConsole',
        () => {
    ...
    const validateSyntaxCommand = vscode.commands.registerCommand(
        'github-actions-dsl.validateSyntax',
        validateSyntax
    );

    // Show AST command
    const showAstCommand = vscode.commands.registerCommand(
        'github-actions-dsl.showAst',
        showAst
    );

    // Get program name for debugger
    const getProgramNameCommand = vscode.commands.registerCommand(
        'github-actions-dsl.getProgramName',
        async () => {
    ...
    context.subscriptions.push(
        startDapServerCommand,
        stopDapServerCommand,
        restartDapServerCommand,
        openDebugConsoleCommand,
        validateSyntaxCommand,
        showAstCommand,
        getProgramNameCommand
    );
}
...
async function startDapServer(): Promise<void> {
    if (dapServerProcess) {
    ...
    const config = vscode.workspace.getConfiguration('github-actions-dsl');
    const executable = config.get<string>('dapServer.executable', './bin/dap');
    const port = config.get<number>('dapServer.port', 4711);
    ...
    if (!fs.existsSync(executablePath)) {
        const buildFirst = await vscode.window.showWarningMessage(
            `DAP server executable not found at ${executablePath}. Would you like to build it first?`,
            'Build', 'Cancel'
        );
    ...
    try {
        ...
        dapServerProcess = spawn(executablePath, args, {
...
async function stopDapServer(): Promise<void> {
    if (dapServerProcess) {
...
async function buildDapServer(): Promise<void> {
    const workspaceFolders = vscode.workspace.workspaceFolders;
    if (!workspaceFolders) {
        return;
    }
    ...
    return new Promise((resolve, reject) => {
        ...
        exec('go build -o bin/dap ./cmd/dap', {
            cwd: workspaceFolders[0].uri.fsPath
        }, (error, stdout, stderr) => {
            if (error) {
                ...
                if (stdout) outputChannel.appendLine(`Build output: ${stdout}`);
                if (stderr) outputChannel.appendLine(`Build stderr: ${stderr}`);
                resolve();
            }
        });
    });
...
async function validateSyntax(): Promise<void> {
    ...
    exec(`"${mainExecutable}" compile "${filePath}" --validate`, {
        cwd: workspaceFolders[0].uri.fsPath
    }, (error, stdout, stderr) => {
        if (error) {
        ...
        if (stdout) outputChannel.appendLine(`Validation output: ${stdout}`);
        if (stderr) outputChannel.appendLine(`Validation stderr: ${stderr}`);
    });
}
...
function getAstVisualizationHtml(): string {
    return `
        <!DOCTYPE html>
        <html lang="en">
        <head>
...
function validateDocumentSyntax(document: vscode.TextDocument, diagnostics: vscode.DiagnosticCollection): void {
    const config = vscode.workspace.getConfiguration('github-actions-dsl');
        ...
        if (line.trim().startsWith('name:') && line.indexOf(':') === line.lastIndexOf(':')) {
            ...
            if (!nameValue) {
                const diagnostic = new vscode.Diagnostic(
                    new vscode.Range(index, line.indexOf(':') + 1, index, line.length),
...
class DslCompletionProvider implements vscode.CompletionItemProvider {
    provideCompletionItems(
        ...
        items.push(
            ...
            new vscode.CompletionItem('jobs', vscode.CompletionItemKind.Keyword),
            new vscode.CompletionItem('steps', vscode.CompletionItemKind.Keyword),
            new vscode.CompletionItem('runs-on', vscode.CompletionItemKind.Keyword),
            new vscode.CompletionItem('uses', vscode.CompletionItemKind.Keyword),
            new vscode.CompletionItem('with', vscode.CompletionItemKind.Keyword),
            new vscode.CompletionItem('env', vscode.CompletionItemKind.Keyword)
        );
...
class DslHoverProvider implements vscode.HoverProvider {
    provideHover(
        document: vscode.TextDocument,
        position: vscode.Position
